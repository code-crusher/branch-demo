...
function G(a){""!==r&&a.setAttribute("nonce",r)};function H(a,b){return function(c,d,e){if(v)return!1;if("number"===typeof e||e)if(0===b){if("object"!==typeof e)return w("API request , parameter  is not ",[c,d,"an object"])}else if(3===b){if(!(e instanceof Array))return w("API request , parameter  is not ",[c,d,"an array"])}else if(2===b){if("number"!==typeof e)return w("API request , parameter  is not ",[c,d,"a number"])}else if(4===b){if("boolean"!==typeof e)return w("API request , parameter  is not ",[c,d,"a boolean"])}else{if("string"!==
typeof e)return w("API request , parameter  is not ",[c,d,"a string"]);if(1!==b&&!b.test(e))return w("API request , parameter  is not ",[c,d,"in the proper format"])}else if(a)return w("API request  missing parameter ",[c,d]);return!1}}var I=/^[0-9]{15,20}$/;function J(a){return z(a,{browser_fingerprint_id:H(!0,I),identity_id:H(!0,I),sdk:H(!0,1),session_id:H(!0,I)})}
...
function Eb(a,b,c){function d(a,c){"undefined"===typeof c&&(c={});if(a.branch_key&&l.test(a.branch_key))return c.branch_key=a.branch_key,c;if(a.app_id&&g.test(a.app_id))return c.app_id=a.app_id,c;if(a.instrumentation)c.instrumentation=a.instrumentation;else throw Error(w("API request  missing parameter ",[b.endpoint,"branch_key or app_id"]));}var e,f,k=b.destination+b.endpoint,g=/^[0-9]{15,20}$/,l=/key_(live|test)_[A-Za-z0-9]{32}/;if("/v1/has-app"===b.endpoint)try{b.f=d(c,b.f)}catch(p){return{error:p.message}}if("undefined"!==
...
function uc(a){Jb(document.body,"branch-banner-is-active");Yb&&"fixed"===Xb&&Jb(document.body,"branch-banner-no-scroll");setTimeout(function(){"top"===O?a.style.top="0":"bottom"===O&&(T.Aa.safeAreaRequired?($b=!0,document.getElementById("branch-banner-iframe").style.transition="all 0s",vc(!1,!0),window.addEventListener("resize",wc),window.addEventListener("scroll",xc)):a.style.bottom="0");N(S,"didShowJourney",T);kc=!0},20)}function wc(){$b&&vc(!1,!1)}
