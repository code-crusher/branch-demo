...
function Ob(a,b){var c=a.get("hideBanner",!0);if(b.qa&&navigator&&Number(navigator.doNotTrack))return!1;try{"string"===typeof c&&(c=n(c))}catch(e){c=!1}var c="number"===typeof c?new Date>=new Date(c):!c,d=b.K;"number"===typeof d&&(d=!1);return!document.getElementById("branch-banner")&&!document.getElementById("branch-banner-iframe")&&(c||d)&&(b.sa&&!A()||b.O&&"android"===A()||b.ta&&"ipad"===A()||b.T&&"ios"===A()||b.P&&"blackberry"===A()||b.S&&"windows_phone"===A()||b.R&&"kindle"===A())};function Pb(a,b){return"#branch-banner-iframe { position: "+("top"!==b||a?"fixed":"absolute")+"; }\n"}
...
function Sb(a,b){var c='<div id="branch-sms-form-container">'+(A()?'<a id="branch-mobile-action" class="button" href="#" target="_parent">'+((L(b)||{}).has_app?a.na:a.ia)+"</a>":'<div class="branch-icon-wrapper" id="branch-loader-wrapper" style="opacity: 0;"><div id="branch-spinner"></div></div><div id="branch-sms-block"><form id="sms-form"><input type="phone" aria-label="Enter phone number" class="branch-animation" name="branch-sms-phone" id="branch-sms-phone" placeholder="'+a.pa+'"><button type="submit" id="branch-sms-send" class="branch-animation button">'+
...
c()},270),setTimeout(function(){"top"===b.position?document.body.style.marginTop=m:"bottom"===b.position&&(document.body.style.marginBottom=t);Kb("branch-banner-is-active")},20))}function f(){"top"===b.position?k.style.top="0":"bottom"===b.position&&(k.style.bottom="0");N(a,"didShowBanner")}if(!Ob(d,b))return N(a,"willNotShowBanner"),null;N(a,"willShowBanner");var k=Sb(b,d);Qb(b,k);c.channel=c.channel||"app banner";var g=b.D?k.contentWindow.document:document;if(A()){b.open_app=b.oa;b.append_deeplink_path=
b.ba;b.make_new_link=b.la;b.deepview_type="banner";a.deepview(c,b);var l=g.getElementById("branch-mobile-action");l&&(l.onclick=function(b){b.preventDefault();a.deepviewCta()})}else g.getElementById("sms-form")?g.getElementById("sms-form").addEventListener("submit",function(d){d.preventDefault();Tb(g,a,b,c)}):k.onload=function(){g=k.contentWindow.document;g.getElementById("sms-form")&&g.getElementById("sms-form").addEventListener("submit",function(d){d.preventDefault();Tb(g,a,b,c)})};var l=Mb("margin-top"),
...
function uc(a){Jb(document.body,"branch-banner-is-active");Yb&&"fixed"===Xb&&Jb(document.body,"branch-banner-no-scroll");setTimeout(function(){"top"===O?a.style.top="0":"bottom"===O&&(alert(T.ka.safeAreaRequired),T.ka.Ca?($b=!0,document.getElementById("branch-banner-iframe").style.transition="all 0s",vc(!1,!0),window.addEventListener("resize",wc),window.addEventListener("scroll",xc)):a.style.bottom="0");N(S,"didShowJourney",T);kc=!0},20)}function wc(){$b&&vc(!1,!1)}
